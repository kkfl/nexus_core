...
async def test_update_deployment(registry_headers):
    async with AsyncClient(app=app, base_url="http://test") as ac:
        res = await ac.post("/v1/agents", json={"name": "test-update-dep-agent"}, headers=registry_headers)
        agent_id = res.json()["id"]

        dep_data = {
            "agent_id": agent_id,
            "env": "dev",
            "base_url": "http://dev-agent:8080"
        }
        res_dep = await ac.post("/v1/deployments", json=dep_data, headers=registry_headers)
        dep_id = res_dep.json()["id"]

        update_data = {"status": "inactive", "base_url": "http://dead-agent:8080"}
        res_patch = await ac.patch(f"/v1/deployments/{dep_id}", json=update_data, headers=registry_headers)
