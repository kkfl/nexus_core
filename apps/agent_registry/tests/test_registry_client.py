...
async def test_resolve_agent_exact_tenant():
    client = AgentRegistryClient(registry_base_url="http://fake-registry", agent_key="key", service_id="test")

    # Mock agent lookup
    respx.get("http://fake-registry/v1/agents/demo-agent").mock(
        return_value=Response(200, json={"id": "agent-123", "name": "demo-agent", "status": "active"})
    )
...
async def test_resolve_agent_disabled():
    ...
    respx.get("http://fake-registry/v1/agents/demo-agent").mock(
        return_value=Response(200, json={"id": "agent-123", "name": "demo-agent", "status": "disabled"})
    )
