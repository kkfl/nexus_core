...
async def replay_dlq_run(
    ...
    from apps.automation_agent.models import AutomationRun, AutomationDLQ
    ...
    if run.dlq_entry:
        ...
        run.dlq_entry.last_replay_at = datetime.now(timezone.utc)
    ...
    await write_audit_event(
        ...
        action="replay_dlq", result="success",
        tenant_id=run.tenant_id, env=run.env,
        automation_id=run.automation_id, run_id=run.id
    )
