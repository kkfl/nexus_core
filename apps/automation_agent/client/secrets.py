...
async def fetch_secret(
    ...
    url = f"{agent.base_url}/v1/secrets/{secret_alias}"
    headers = {
        "X-Service-ID": "automation-agent",
        "X-Agent-Key": config.automation_agent_keys.get("automation-agent", "internal-automation-key"), # Usually vault gives each agent their own key
    ...
    try:
        async with httpx.AsyncClient(timeout=5.0) as client:
            ...
            read_resp = await client.post(read_url, headers=headers, json={"reason": "automation agent execution"})
