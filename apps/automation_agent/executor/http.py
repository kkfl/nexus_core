...
async def execute_agent_action(
    ...
    if agent.auth_scheme == "headers":
        if not agent.auth_secret_alias:
            return False, f"Agent {agent_name} requires header auth but no secret alias is registered in registry."
            
        secret_val = await fetch_secret(agent.auth_secret_alias, tenant_id, env, correlation_id)
        if not secret_val:
            return False, f"Failed to retrieve auth secret '{agent.auth_secret_alias}' from secrets-agent."
    ...
    try:
        async with httpx.AsyncClient(timeout=timeout_seconds) as client:
            ...
            if resp.is_error:
                logger.warning("agent_action_failed", agent=agent_name, status=resp.status_code, response=str(resp_data)[:200])
