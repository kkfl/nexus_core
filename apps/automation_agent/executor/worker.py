...
from apps.automation_agent.store.postgres import (
    create_dlq_entry,
    create_step_runs,
    get_pending_runs,
)
...
async def process_run(db, run: AutomationRun):
    ...
    logger.info(
        "processing_run_started", run_id=run.id, automation_id=automation.id, steps=len(steps)
    )
    ...
    if run_success:
        ...
        if automation.notify_on_success:
            await send_notification(
                ...
                template_id="generic",
                context={
                    "subject": f"Automation Succeeded: {automation.name}",
                    "body": f"Run {run.id} completed successfully.",
                },
                correlation_id=run.correlation_id,
            ...
            try:
                async for db in get_db_session_func():
                    ...
                    stmt = (
                        ...
                        .options(joinedload(AutomationRun.automation))
