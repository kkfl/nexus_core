...
from apps.automation_agent.api import audit, automations, dlq, runs
from apps.automation_agent.client.registry import get_registry_client, resolve_agent
    ...
    try:
        ...
        while time.time() - start < 30:
            ...
            for agent in vital_agents:
                try:
                    await resolve_agent(agent, "nexus", "prod")
                except Exception:
                    all_ready = False
                    break
            if all_ready:
...
async def lifespan(app: FastAPI):
    ...
    scheduler_task = asyncio.create_task(
        scheduler_loop(session_generator, tick_interval_sec=config.cron_tick_interval_seconds)
    )
    worker_task = asyncio.create_task(
        worker_loop(
            session_generator, tick_interval_sec=3, concurrency=config.max_concurrent_runs_global
        )
    )
