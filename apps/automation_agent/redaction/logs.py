...
CRED_PATTERN = re.compile(r'(?i)(password|secret|token|key|pwd|auth|credential)[\s\=\:]+[\'"]?([^\s\'"]+)[\'"]?')
...
def redact_dict(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for k, v in data.items():
        ...
        if any(term in k_lower for term in ['password', 'secret', 'token', 'key', 'auth']):
            ...
            redacted[k] = [redact_dict(i) if isinstance(i, dict) else (redact_string(str(i)) if isinstance(i, str) else i) for i in v]
