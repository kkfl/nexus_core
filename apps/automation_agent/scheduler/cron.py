...
from apps.automation_agent.store.postgres import get_due_cron_automations, create_run
    ...
    try:
        ...
        for auto in automations:
            try:
                ...
                if prev.year == now.year and prev.month == now.month and prev.day == now.day and prev.hour == now.hour and prev.minute == now.minute:
                    ...
                    try:
                        ...
                        logger.info("cron_run_enqueued", automation_id=auto.id, run_id=run.id, idempotency_key=idempotency_key)
                        
                        # Note: we flush to db within the loop so the DB unique constraint prevents duplicate creation
                        await db.commit()
                        
                    except Exception as ex:
                        if "unique constraint" in str(ex).lower() or "duplicate key" in str(ex).lower():
                            # Already enqueued this minute by another worker tick
                            await db.rollback()
                        else:
                            logger.error("cron_run_enqueue_failed", automation_id=auto.id, error=str(ex))
                            await db.rollback()
