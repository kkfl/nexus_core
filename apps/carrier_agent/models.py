...
from sqlalchemy import (
    Boolean, Column, DateTime, ForeignKey, Integer, String, JSON, Text, UniqueConstraint
)
...
class CarrierDidInventory(Base):
    ...
    carrier_target_id = Column(String(36), ForeignKey("carrier_targets.id", ondelete="CASCADE"), nullable=False)
    ...
    capabilities = Column(JSON, nullable=False, default=dict) # {"voice": True, "sms": False, "mms": False}
...
class CarrierJob(Base):
    ...
    status = Column(String(50), nullable=False, default="pending", index=True) # pending, running, succeeded, failed
