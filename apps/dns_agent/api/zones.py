...
async def create_zone(
    ...
    zone = await store.create_zone(db, payload.tenant_id, payload.env, payload.zone_name, payload.provider)

    # Attempt to resolve provider zone ID immediately (non-fatal if vault unavailable)
    try:
        vault = dns_vault_client_from_env()
        adapter = await get_adapter(payload.provider, payload.tenant_id, payload.env, vault,
    ...
    await store.log_audit(
        db, correlation_id=identity.correlation_id,
        service_id=identity.service_id,
        tenant_id=payload.tenant_id, env=payload.env,
        action="create_zone", zone_name=payload.zone_name,
        result="success", ip_address=identity.ip_address,
    )
