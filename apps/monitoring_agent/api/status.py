...
from apps.monitoring_agent.store.postgres import (
...
async def check_history(
    ...
    stmt = (
        ...
        .join(MonitoringTarget, MonitoringCheck.target_id == MonitoringTarget.id)
        .order_by(desc(MonitoringCheck.started_at))
        .limit(limit)
    )
...
async def list_incidents(
    ...
    stmt = (
        ...
        .where(MonitoringAuditEvent.action.in_(["state_change", "alert_dispatched"]))
        .order_by(desc(MonitoringAuditEvent.created_at))
        .limit(limit)
    )
...
async def dispatch_daily_digest(
    ...
    if tenant_id:
        inc_stmt = inc_stmt.where(MonitoringAuditEvent.tenant_id == tenant_id)
    if env:
        inc_stmt = inc_stmt.where(MonitoringAuditEvent.env == env)
    ...
    dummy = MonitoringTarget(
        tenant_id=tenant_id, env=env, agent_name="Nexus Platform Root", base_url=""
    )
