...
async def dispatch_alert(
    target: MonitoringTarget, severity: str, message: str, correlation_id: str
):
    ...
    try:
        ...
        alias = (
            "monitoring-agent.automation-agent.key"  # The established system secret for notifying
        )
        ...
        async with httpx.AsyncClient() as client:
            resp = await client.post(
                f"{settings.notifications_base_url}/v1/notify",
                json=payload,
                headers=headers,
                timeout=5.0,
            )
            if resp.status_code >= 400:
                logger.error(
                    "failed_to_dispatch_alert", status_code=resp.status_code, body=resp.text
                )
            else:
                logger.info(
                    "alert_dispatched_successfully", target=target.agent_name, severity=severity
                )
