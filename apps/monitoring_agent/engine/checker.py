...
async def run_checks(
    db: AsyncSession, tenant_id: str = None, env: str = None, correlation_id: str = None
):
    ...
    successes = 0
    for target, result in zip(targets, http_results, strict=False):
            ...
            if resp_c.status_code == 200:
                ...
                try:
                    ...
                    capabilities_hash = hashlib.sha256(
                        json.dumps(cap_data, sort_keys=True).encode()
                    ).hexdigest()
                except Exception:
                    pass
...
async def _process_check_result(
    db: AsyncSession, target: MonitoringTarget, correlation_id: str, result_data: tuple
) -> bool:
    (
        health_ok,
        ready_ok,
    ...
    await evaluate_and_alert(
        db, target, state, correlation_id, is_ok, error_detail, capabilities_hash
    )
    ...
    if not is_ok:
        metrics.inc(
            "monitoring_failures_total", agent=target.agent_name, reason=error_code or "unknown"
        )
