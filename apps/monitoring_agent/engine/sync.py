...
from apps.monitoring_agent.store.postgres import upsert_target, log_audit
    ...
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            if resp.status_code != 200:
                ...
                await log_audit(db, correlation_id, "monitoring-agent", None, "prod", "sync_registry", "failed", detail=f"HTTP {resp.status_code}")
                return 0
            ...
            for d in deployments:
                target_dict = {
                    ...
                    "env": d.get("env", "prod"),
                    "agent_name": d.get("agent_name", "unknown"), # Technically registry GET /deployments returns agent_id, but the V1 schema joins them.
            ...
            agents_resp = await client.get(f"{settings.registry_base_url}/v1/agents", headers=headers)
            agents = {a["id"]: a["name"] for a in agents_resp.json()} if agents_resp.status_code == 200 else {}
            ...
            await log_audit(db, correlation_id, "monitoring-agent", None, "prod", "sync_registry", "success", detail=f"Synced {synced_count} deployments")
            await db.commit()
            return synced_count
            
    except Exception as e:
        logger.error("registry_sync_exception", error=str(e))
        await log_audit(db, correlation_id, "monitoring-agent", None, "prod", "sync_registry", "error", detail=str(e))
        return 0
