...
from packages.shared.models import ApiKey, Agent, Secret, AuditEvent, PbxTarget
...
class CarrierTargetInternalOut(BaseModel):
    ...
    api_secret_secret_id: Optional[str]

router = APIRouter()

api_key_header = APIKeyHeader(name="X-Nexus-Internal", auto_error=True)

async def _get_internal_agent(api_key: str = Security(api_key_header), db: AsyncSession = Depends(get_db)):
    res = await db.execute(select(ApiKey).where(ApiKey.key == api_key, ApiKey.is_active == True))
...
async def decrypt_internal_secret(
    ...
    authorized = False
    if secret.owner_type == "agent" and secret.owner_id == current_agent.id:
        authorized = True
    # Option 2: secret is a PBX AMI secret (any agent can decrypt for now as long as it has auth via internal route)
    # Ideally, we verify they were dispatched a task referencing this PBX target.
    # For MVP V1, if they have an internal API key and they ask for a pbx_target secret, we allow it.
    elif secret.owner_type == "pbx_target":
        authorized = True
    elif secret.purpose == "storage_target_auth":
        authorized = True
    elif secret.purpose == "carrier_target_auth":
        authorized = True
        
    if not authorized:
        # Audit denial
        db.add(AuditEvent(actor_type="agent", actor_id=current_agent.id, action="secret_decrypt_denied", target_type="secret", target_id=0, meta_data={"secret_id": req.secret_id}))
        await db.commit()
    ...
    db.add(AuditEvent(actor_type="agent", actor_id=current_agent.id, action="secret_decrypt", target_type="secret", target_id=0, meta_data={"secret_id": req.secret_id}))
    await db.commit()
