...
from packages.shared.models import KbSource, KbDocument, KbChunk, KbEmbedding, KbAccessLog, User, Agent, PersonaVersion
...
from apps.nexus_api.dependencies import get_current_user, get_current_identity, RequireRole
...
async def delete_document(
    ...
    await db.delete(doc) # hard delete with cascade if cascading was on, but we don't have constraints set for cascade.
...
async def read_chunks(
    ...
    res = await db.execute(select(KbChunk).where(KbChunk.document_id == id).offset(skip).limit(limit))
    return res.scalars().all()
...
async def enforce_rag_policy(
    ...
    if persona_version_id:
        res = await db.execute(select(PersonaVersion).where(PersonaVersion.id == persona_version_id))
    ...
    if pv_to_check and pv_to_check.tools_policy:
        ...
        for ns in namespaces:
            ...
            if any((a == "*" or a == ns or (a.endswith("*") and ns.startswith(a[:-1]))) for a in allowed):
