...
from apps.nexus_api.dependencies import get_current_identity
from packages.shared import metrics as metrics_emitter
from packages.shared.db import get_db
from packages.shared.models import Agent, PersonaVersion, Task, TaskRoute, TaskRun, User
...
async def create_task(
    ...
    if task_in.persona_version_id:
        pv_res = await db.execute(
            select(PersonaVersion).where(PersonaVersion.id == task_in.persona_version_id)
        )
    ...
    await metrics_emitter.emit(
        db, "task_create", meta={"task_type": db_task.type, "task_id": db_task.id}
    )
    await db.commit()
