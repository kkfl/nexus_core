...
from packages.shared.models import Task, Agent, PersonaVersion, Artifact, TaskRoute, PersonaDefault, ApiKey, KbDocument, KbChunk, KbEmbedding, Secret, Entity, EntityEvent, TaskLink, IdempotencyKey, AuditEvent, StorageJob, CarrierSnapshot
import uuid
import hashlib
from packages.shared.sor import validate_proposed_write, check_idempotency, apply_json_merge_patch, SoRValidationError
from packages.shared.storage import get_storage_backend
from packages.shared.schemas.agent_sdk import AgentTaskRequest, PersonaBlock, AgentTaskMetadata, AgentTaskResponse
...
async def _dispatch_task(task_id: int, attempt: int = 1):
    async with get_db_context() as db:
        ...
        if not persona_version_id:
            ...
            pd_res = await db.execute(select(PersonaDefault).where(
                PersonaDefault.scope_type == 'task_type',
                PersonaDefault.scope_value == task.type,
                PersonaDefault.is_active == True
            ))
        ...
        if persona_version_id:
            pv_res = await db.execute(select(PersonaVersion).where(PersonaVersion.id == persona_version_id))
        ...
        # Find Route
        route_res = await db.execute(select(TaskRoute).where(
            TaskRoute.task_type == task.type,
            TaskRoute.is_active == True
        ))
        ...
        if persona_tools is not None:
            try:
                ...
                task.status = "failed"
                await metrics_emitter.emit(db, "persona_policy_violation", meta={"task_id": task.id, "type": task.type, "detail": e.detail})
                await db.commit()
        ...
        if task.assigned_agent_id:
            agent_res = await db.execute(select(Agent).where(Agent.id == task.assigned_agent_id, Agent.is_active == True))
            agent = agent_res.scalars().first()
        else:
            # Find eligible agents
            eligibles_res = await db.execute(select(Agent).where(Agent.is_active == True))
        ...
        if final_timeout_seconds is None or (agent.timeout_seconds and agent.timeout_seconds < final_timeout_seconds):
        ...
        if not persona_version_id:
            ...
            if not pd:
                pd_res = await db.execute(select(PersonaDefault).where(
                    PersonaDefault.scope_type == 'global',
                    PersonaDefault.is_active == True
                ))
                pd = pd_res.scalars().first()
            if pd:
                pv_res = await db.execute(select(PersonaVersion).where(PersonaVersion.id == pd.persona_version_id))
                pv = pv_res.scalars().first()
                if pv:
                    persona_data = PersonaBlock(
                        id=str(pv.id),
        ...
        running_tasks_res = await db.execute(select(Task).where(Task.assigned_agent_id == agent.id, Task.status == "running"))
        running_count = len(running_tasks_res.scalars().all())
        ...
        if agent.auth_type == "api_key":
            ...
            sec_res = await db.execute(select(Secret).where(
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if parsed_res.ok:
                    ...
                    await metrics_emitter.emit(db, "task_succeeded", meta={"task_id": task.id, "type": task.type, "agent_id": agent.id})
                    ...
                    if getattr(parsed_res, "proposed_writes", None):
                        try:
                            for pw in parsed_res.proposed_writes:
                                ...
                                stored_response = await check_idempotency(db, pw.idempotency_key, "entity_write", pg_dict)
                                if stored_response:
                                ...
                                if pw.external_ref:
                                    res_ent = await db.execute(select(Entity).where(Entity.kind == pw.entity_kind, Entity.external_ref == pw.external_ref))
                                ...
                                db.add(AuditEvent(actor_type="agent", actor_id=agent.id, action=f"entity_{action}", target_type="entity", target_id=0, meta_data={"entity_id": db_ent.id}))
                            ...
                            if getattr(parsed_res, "proposed_tasks", None):
                                if not persona_version_id or not persona_data:
                                    raise SoRValidationError("proposed_tasks rejected: persona inactive or missing")
                                    
                                if "monitoring.alert_to_task" not in persona_tools.get("allowed_capabilities", []):
                                     raise SoRValidationError(f"proposed_tasks rejected: persona lacks monitoring.alert_to_task capability")
                                     
                                for pt in parsed_res.proposed_tasks:
                                    idem_res = await db.execute(select(IdempotencyKey).where(IdempotencyKey.key == pt.idempotency_key))
                            ...
                            if getattr(parsed_res, "job_summary", None):
                                ...
                                if target_id and parsed_res.job_summary.kind in ("copy", "lifecycle_propose", "lifecycle_apply", "delete"):
                                    new_job = StorageJob(
                                        id=str(uuid.uuid4()),
                                    ...
                                    db.add(new_job)
                                    db.add(AuditEvent(actor_type="agent", actor_id=agent.id, action=f"storage_job_{parsed_res.job_summary.status}", target_type="storage_job", target_id=0, meta_data={"job_id": new_job.id, "kind": new_job.kind}))
                                
                                carrier_target_id = task.payload.get("carrier_target_id")
                                if carrier_target_id and parsed_res.job_summary.kind == "carrier_snapshot":
                                    ...
                                    db.add(new_snapshot)
                                    db.add(AuditEvent(actor_type="agent", actor_id=agent.id, action=f"carrier_snapshot_{parsed_res.job_summary.status}", target_type="carrier_snapshot", target_id=0, meta_data={"snapshot_id": new_snapshot.id}))
...
async def _embed_document(document_id: int):
    async with get_db_context() as db:
        ...
        try:
            ...
            old_chunks_res = await db.execute(select(KbChunk.id).where(KbChunk.document_id == doc.id))
            old_chunk_ids = [c for c in old_chunks_res.scalars().all()]
            if old_chunk_ids:
                await db.execute(KbEmbedding.__table__.delete().where(KbEmbedding.chunk_id.in_(old_chunk_ids)))
