...
def _job_to_out(job) -> JobOut:
    ...
    for d in (job.deliveries or []):
        deliveries.append(DeliveryOut(
            id=str(d.id), channel=d.channel, status=d.status,
            destination_hash=d.destination_hash, provider_msg_id=d.provider_msg_id,
            attempt=d.attempt, error_code=d.error_code, sent_at=d.sent_at,
        ))
    return JobOut(
        id=str(job.id), tenant_id=job.tenant_id, env=job.env,
        severity=job.severity, status=job.status, channels=job.channels,
        template_id=job.template_id, sensitivity=job.sensitivity,
        attempts=job.attempts, correlation_id=job.correlation_id,
...
async def notify(
    ...
    if existing:
        await write_audit(db, correlation_id=correlation_id, service_id=identity.service_id,
                          tenant_id=req.tenant_id, env=req.env, action="notify",
                          result="dedup", job_id=str(existing.id), ip_address=identity.ip_address)
        return NotifyResponse(job_id=str(existing.id), status=existing.status,
    ...
    channels, rule_config = await resolve_channels(
        db, tenant_id=req.tenant_id, env=req.env, severity=req.severity,
    ...
    job = await store.create_job(
        db,
        tenant_id=req.tenant_id, env=req.env, severity=req.severity,
        channels=channels, idempotency_key=req.idempotency_key,
        correlation_id=correlation_id, created_by_service_id=identity.service_id,
        template_id=req.template_id, subject=subject,
        body_hash=body_hash, body_stored=body_stored,
        sensitivity=req.sensitivity, context=req.context,
    )
    job_id = str(job.id)

    await write_audit(db, correlation_id=correlation_id, service_id=identity.service_id,
                      tenant_id=req.tenant_id, env=req.env, action="notify",
                      result="ok", job_id=job_id, ip_address=identity.ip_address)
    ...
    await dispatch_job(
        job_id=job_id, tenant_id=req.tenant_id, env=req.env,
        channels=channels, subject=subject, body=body,
        correlation_id=correlation_id,
        destinations=req.destinations,
        channel_configs=channel_configs,
    )

    return NotifyResponse(job_id=job_id, status="pending",
...
async def replay_job(
    ...
    if job.status not in ("failed", "partial"):
        raise HTTPException(status_code=409, detail=f"Job status is '{job.status}'; only failed/partial jobs can be replayed.")

    body = job.body_stored or ""
    if not body:
        raise HTTPException(status_code=422, detail="Cannot replay: body was not stored (sensitivity=sensitive).")

    await store.set_job_status(db, job_id, "pending")
    await dispatch_job(
        job_id=job_id, tenant_id=job.tenant_id, env=job.env,
        channels=job.channels, subject=job.subject, body=body,
