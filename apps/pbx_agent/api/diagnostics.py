...
from apps.pbx_agent.ops.diagnostics import (
    diagnostic_ping, diagnostic_ami_check, diagnostic_version
)
...
async def ping(
    ...
    await write_audit_event(db, req.correlation_id or identity.correlation_id, identity.service_id,
                            "diagnostics.ping", "success",
                            tenant_id=req.tenant_id, env=req.env, target_id=target.id)
    await db.commit()
    return result
...
async def ami_check(
    ...
    try:
        ...
        await write_audit_event(db, identity.correlation_id, identity.service_id,
                                "diagnostics.ami_check", "error",
                                tenant_id=req.tenant_id, env=req.env, target_id=target.id,
                                detail=str(e))
        await db.commit()
        raise HTTPException(status_code=502, detail=str(e))

    result = await diagnostic_ami_check(target.host, target.ami_port, target.ami_username, ami_secret)
    result_label = "success" if result.get("auth_ok") else "error"
    await write_audit_event(db, identity.correlation_id, identity.service_id,
                            "diagnostics.ami_check", result_label,
                            tenant_id=req.tenant_id, env=req.env, target_id=target.id)
    await db.commit()
    return result
...
async def version(
    ...
    try:
        result = await diagnostic_version(target.host, target.ami_port, target.ami_username, ami_secret)
    except AmiError as e:
        raise HTTPException(status_code=502, detail=str(e))

    await write_audit_event(db, identity.correlation_id, identity.service_id,
                            "diagnostics.version", "success",
                            tenant_id=req.tenant_id, env=req.env, target_id=target.id)
    await db.commit()
    return result
