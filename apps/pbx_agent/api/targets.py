...
from apps.pbx_agent.adapters.ssh_system import (
...
from apps.pbx_agent.client.secrets import (
    SecretsError,
    delete_secret_by_alias,
    fetch_secret,
    store_secret,
)
from apps.pbx_agent.schemas import (
    PbxRegistrationResult,
    PbxTargetCreate,
    PbxTargetEdit,
    PbxTargetOut,
    PbxTargetRegister,
...
async def register_and_verify(
    ...
    try:
        ...
        checks.append(
            VerifyCheckResult(check="Vault: AMI secret stored", passed=False, detail=str(e))
        )
    ...
    if payload.ssh_key_pem:
        try:
            await store_secret(
                alias=ssh_key_alias,
                value=sanitize_ssh_key(payload.ssh_key_pem),
                tenant_id=payload.tenant_id,
                env=payload.env,
            ...
            checks.append(
                VerifyCheckResult(check="Vault: SSH key stored", passed=False, detail=str(e))
            )

    if payload.ssh_password:
        try:
            await store_secret(
                alias=ssh_pass_alias,
                value=payload.ssh_password,
                tenant_id=payload.tenant_id,
                env=payload.env,
            ...
            checks.append(
                VerifyCheckResult(check="Vault: SSH password stored", passed=False, detail=str(e))
            )
    ...
    if not ssh_reachable:
        ...
        if payload.ssh_password:
            try:
                ...
                if snap.ssh_ok:
                    checks.append(
                        VerifyCheckResult(check="SSH: Password authentication", passed=True)
                    )
                else:
                    checks.append(
                        VerifyCheckResult(
    ...
    if not ami_reachable:
        ...
        login_ok = await ami.check_ami_login(
            host=payload.host,
            port=payload.ami_port,
            username=payload.ami_username,
            secret=payload.ami_secret,
        )
        checks.append(
            VerifyCheckResult(
                check="AMI: Login successful",
                passed=login_ok,
...
async def verify_target(
    ...
    try:
        ...
        checks.append(
            VerifyCheckResult(check="Vault: AMI secret readable", passed=False, detail=str(e))
        )
    ...
    if target.ssh_key_alias:
        try:
            ...
            checks.append(
                VerifyCheckResult(check="Vault: SSH key readable", passed=False, detail=str(e))
            )
    ...
    if target.ssh_password_alias:
        try:
            ...
            checks.append(
                VerifyCheckResult(check="Vault: SSH password readable", passed=False, detail=str(e))
            )
        ...
        if ami_secret:
            ...
            checks.append(
                VerifyCheckResult(
                    ...
                    detail=None
                    if login_ok
                    else "Authentication failed — check AMI username/secret",
                )
            )
...
async def verify_target_stream(
    ...
    async def _generate():
        ...
        if not ssh_reachable:
            yield _evt(
                "SSH: Port reachable", False, f"Cannot reach {target.host}:{target.ssh_port}"
            )
        else:
        ...
        if not ami_reachable:
            yield _evt(
                "AMI: Port reachable", False, f"Cannot reach {target.host}:{target.ami_port}"
            )
        else:
...
async def delete_target(
    ...
    aliases_to_delete = [
        a for a in [target.ami_secret_alias, target.ssh_key_alias, target.ssh_password_alias] if a
    ]
    deleted_secrets = 0
