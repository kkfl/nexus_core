...
from sqlalchemy import (
    Boolean, Column, DateTime, Index, Integer, JSON, LargeBinary, String,
...
class VaultSecret(VaultBase):
    ...
    scope_tags: Mapped[Optional[Dict[str, Any]]] = mapped_column(JSON, server_default="[]")
    ...
    last_rotated_at: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime(timezone=True))
    next_due_at: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime(timezone=True))
...
class VaultPolicy(VaultBase):
    ...
    tenant_id: Mapped[Optional[str]] = mapped_column(String(128))  # None = all tenants
    env: Mapped[Optional[str]] = mapped_column(String(32))         # None = all envs
...
class VaultAuditEvent(VaultBase):
    ...
    ts: Mapped[datetime.datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), index=True)

    __table_args__ = (
        Index("ix_vault_audit_ts_service", "ts", "service_id"),
    )
