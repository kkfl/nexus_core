...
async def create_server(body: CreateServerRequest, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        await nc.notify(
            ...
            idempotency_key=f"server-create:{job.id}",
        )
    except Exception:
        logger.warning("telegram_notify_failed", action="server_create")
...
async def delete_server(server_id: str, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        await nc.notify(
            ...
            idempotency_key=f"server-delete:{job.id}",
        )
    except Exception:
        logger.warning("telegram_notify_failed", action="server_delete")
