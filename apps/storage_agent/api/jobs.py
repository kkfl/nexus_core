...
from apps.storage_agent.engine import retention, alerter
...
async def execute_retention_job(req: RetentionJobRequest, background_tasks: BackgroundTasks, db=Depends(postgres.get_db)):
    ...
    async def bg_retention_task():
        ...
        async for s_db in postgres.get_db():
            try:
                ...
                body = (f"Storage Retention {'Dry-Run ' if req.dry_run else ''}Completed\n"
                ...
                await alerter.dispatch_alert(
                    req.tenant_id, req.env, "Storage Retention Passed", body,
                    channel="telegram", correlation_id=correlation_id
                )
                
            except Exception as e:
                logger.error("bg_retention_task_failed", error=str(e), correlation_id=correlation_id)
                # Dispatch error
                body = f"Retention failed on {req.storage_target_id}. Error: {str(e)[:250]}"
                await alerter.dispatch_alert(
                    req.tenant_id, req.env, "Storage Retention FAILED", body,
                    channel="telegram", correlation_id=correlation_id
                )
