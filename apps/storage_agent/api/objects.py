...
async def presign_url(req: PresignRequest, db=Depends(postgres.get_db)):
    ...
    url = await s3.generate_presigned_url(target, req.bucket_name, req.object_key, req.method, ttl, "presign_gen")
    return {"url": url, "expires_in": ttl, "method": req.method.upper(), "object_key": req.object_key}
...
async def put_object(
    ...
    res = await s3.upload_object(target, bucket_name, object_key, data, file.content_type, "put_obj")
    
    # Register Record
    obj_rec = await postgres.register_object(
        db, tenant_id, env, target.id, b.id, object_key,
        file.content_type, res["size_bytes"], tags={},
...
async def list_objects(req: ListObjectsRequest, db=Depends(postgres.get_db)):
    ...
    res = await s3.list_objects(target, req.bucket_name, req.prefix, req.max_keys, req.continuation_token, "list_obj")
    return res
