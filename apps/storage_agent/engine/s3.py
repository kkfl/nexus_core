...
from apps.storage_agent.metrics import observe_latency, inc
...
async def _get_s3_client(target: StorageTarget, correlation_id: str) -> AsyncGenerator[Any, None]:
    ...
    if not alias_ak or not alias_sk:
        logger.error("target_missing_credential_aliases", target_id=target.storage_target_id, correlation_id=correlation_id)
        raise StorageOperationError(f"Target '{target.storage_target_id}' missing alias config for credentials", "missing_config")
    ...
    if not ak or not sk:
        logger.error("credentials_fetch_failed", target_id=target.storage_target_id, correlation_id=correlation_id)
        raise StorageOperationError(f"Failed to fetch credentials for target '{target.storage_target_id}'", "auth_failed")
    ...
    try:
        async with _get_s3_client(target, correlation_id) as s3:
            try:
                ...
                if error_code == '404':
                    logger.info("bucket_not_found_creating", bucket_name=bucket_name, target_id=target.storage_target_id)
        ...
        logger.error("s3_client_error", error=str(e), op="ensure_bucket", correlation_id=correlation_id)
        raise StorageOperationError("Failed to ensure bucket", "s3_error")
    except StorageOperationError:
        raise
    except Exception as e:
        logger.error("internal_error", error=str(e), op="ensure_bucket", correlation_id=correlation_id)
        raise StorageOperationError("Internal server error", "internal_error")

async def upload_object(target: StorageTarget, bucket_name: str, object_key: str, data: bytes, content_type: str, correlation_id: str) -> Dict[str, Any]:
...
async def generate_presigned_url(target: StorageTarget, bucket_name: str, object_key: str, method: str, expires_in: int, correlation_id: str) -> str:
...
async def delete_object(target: StorageTarget, bucket_name: str, object_key: str, correlation_id: str) -> bool:
...
async def list_objects(target: StorageTarget, bucket_name: str, prefix: str, max_keys: int, continuation_token: Optional[str], correlation_id: str) -> Dict[str, Any]:
