...
async def get_secret(
    alias: str, tenant_id: str = "nexus", env: str = "prod", correlation_id: str | None = None
) -> str | None:
    ...
    try:
        async with httpx.AsyncClient(timeout=3.0) as client:
            logger.info(
                "fetching_vault_secrets_list",
                url=f"{settings.vault_base_url}/v1/secrets",
        ...
        logger.error(
            "vault_connection_failure", error=str(e), alias=alias, correlation_id=correlation_id
        )
        return None
