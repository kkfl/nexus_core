...
async def check_agent(client, agent_name, base_url):
    ...
    try:
        ...
        results["healthz"] = f"{r.status_code} {r.text[:30]}"
    except Exception as e:
        results["healthz"] = f"Error"

    try:
        r = await client.get(f"{base_url}/readyz", timeout=3.0)
        results["readyz"] = f"{r.status_code} {r.text[:30]}"
    except Exception as e:
        results["readyz"] = f"Error"

    try:
        r = await client.get(f"{base_url}/capabilities", headers=HEADERS, timeout=3.0)
        if r.status_code == 200:
            results["capabilities"] = f"200 OK"
        else:
            results["capabilities"] = f"{r.status_code}"
    except Exception as e:
        results["capabilities"] = f"Error"
...
async def main():
    async with httpx.AsyncClient() as client:
        ...
        for a in agents:
            ...
            deps = deps_res.json()
            if deps:
                # just pick the first one to avoid duplicates in test
                tasks.append(check_agent(client, a["name"], deps[0]["base_url"]))
                
        results = await asyncio.gather(*tasks)
        
        print(f"{'Agent':<20} | {'URL':<30} | {'Healthz':<25} | {'Readyz':<25} | {'Capabilities'}")
        print("-" * 120)
        for r in results:
            h = r['healthz'].replace('\n', '')
            rd = r['readyz'].replace('\n', '')
            c = r['capabilities'].replace('\n', '')
            print(f"{r['name']:<20} | {r['url']:<30} | {h:<25} | {rd:<25} | {c}")
