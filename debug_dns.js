...
(async () => {
    ...
    page.on('request', request => {
        if (request.url().includes('dns')) {
            console.log(`[REQ] ${request.method()} ${request.url()}`);
        }
    });
    page.on('response', async response => {
        if (response.url().includes('dns')) {
    ...
    try {
        ...
        console.log("Waiting 5s for render crashes...");
        await new Promise(r => setTimeout(r, 5000));
        
        const html = await page.evaluate(() => document.getElementById('root')?.innerHTML || 'NO_ROOT');
        if (html.length < 500) {
            console.log('ROOT IS SUSPICIOUSLY SHORT:', html.length);
        } else {
            console.log('ROOT IS OK, length:', html.length);
        }
        
        console.log("Taking screenshot...");
        await page.screenshot({ path: 'c:/Users/randa/.gemini/antigravity/brain/a7f17d10-a89d-4d1b-a9ac-5930b7a8f743/debug_dns_page.png' });
        console.log("Done.");
