...
def test_carrier_e2e():
    ...
    r2 = requests.post(f"{BASE}/v1/automations/{auto_id}/run", headers=HEADERS, json=run_payload, timeout=5)
    ...
    for _ in range(15):
        time.sleep(2)
        r3 = requests.get(f"{BASE}/v1/runs/{run_id}", headers=HEADERS, params={"tenant_id": "nexus", "env": "prod"}, timeout=5)
    ...
    r4 = requests.get(f"{BASE}/v1/runs/{run_id}/steps", headers=HEADERS, params={"tenant_id": "nexus", "env": "prod"}, timeout=5)
    assert r4.status_code == 200, f"Get steps failed: {r4.status_code} {r4.text}"
    steps = r4.json()
    for s in steps:
        print(f"   - Step [{s['step_id']}] agent={s['target_agent']} status={s['status']} attempt={s['attempt']}")
    ...
    if final_status == "failed":
        print("NOTE: Run FAILED. This is likely expected if Telegram auth is missing or mock target doesn't exist.")
