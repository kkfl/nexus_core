...
headers = {
    "X-Service-ID": "admin",
    "X-Agent-Key": "admin-monitoring-key-change-me"
}
...
async def run_all_green():
    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        ...
        status = resp.json()
        all_up = all([s["state"] == "UP" for s in status if s["agent_name"] in ["agent-registry", "secrets-agent", "notifications-agent", "monitoring-agent"]])
