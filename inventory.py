    ...
    async with httpx.AsyncClient() as client:
        ...
        for expected in EXPECTED_AGENTS:
            if expected["name"] not in agent_names:
                ...
                res = await client.post(f"{REGISTRY_URL}/agents", json={"name": expected["name"]}, headers=HEADERS)
                if res.status_code in (200, 201):
                    agent_id = res.json()["id"]
                    print(f"Adding deployment for: {expected['name']}")
                    await client.post(f"{REGISTRY_URL}/deployments", json={
                        "agent_id": agent_id,
                        "env": "prod",
        ...
        # 3. Output Table
        print(f"{'agent_name':<20} | {'tenant_id':<10} | {'env':<5} | {'base_url':<30} | {'version':<10} | {'auth_scheme':<12} | {'capabilities_endpoint'}")
        print("-" * 120)
        ...
        for a in agents:
            ...
            deps = deps_res.json()
            if not deps:
                print(f"{a['name']:<20} | <none>     | <none> | <none>                         | <none>     | <none>")
            for d in deps:
                tenant = d.get("tenant_id") or "global"
                env = d.get("env") or "prod"
                print(f"{a['name']:<20} | {tenant:<10} | {env:<5} | {d['base_url']:<30} | {'v1':<10} | {d['auth_scheme']:<12} | /capabilities")
