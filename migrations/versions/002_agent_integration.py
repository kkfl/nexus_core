...
def upgrade() -> None:
    ...
    op.add_column(
        "agents", sa.Column("status", sa.String(), server_default="unknown", nullable=False)
    )
    op.add_column(
        "agents", sa.Column("max_concurrency", sa.Integer(), server_default="2", nullable=False)
    )
    op.add_column(
        "agents", sa.Column("timeout_seconds", sa.Integer(), server_default="30", nullable=False)
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "persona_version_id", sa.Integer(), sa.ForeignKey("persona_versions.id"), nullable=False
        ),
        sa.Column("is_active", sa.Boolean(), server_default="true", nullable=False),
        sa.Column("created_at", sa.DateTime(), server_default=sa.text("now()"), nullable=False),
    )
