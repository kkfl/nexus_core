...
def upgrade() -> None:
    ...
    op.create_table(
        'monitoring_ingests',
        sa.Column('id', sa.String(), primary_key=True),
        sa.Column('monitoring_source_id', sa.String(), sa.ForeignKey('monitoring_sources.id'), nullable=False),
    ...
    op.create_index('ix_monitoring_ingests_monitoring_source_id', 'monitoring_ingests', ['monitoring_source_id'])
