...
class AgentRegistryClient:
    def __init__(
        self, registry_base_url: str, agent_key: str, service_id: str = "nexus", timeout: int = 5
    ):
    ...
    async def resolve_agent(
        self, agent_name: str, tenant_id: str | None, env: str
    ) -> ResolvableAgent | None:
        """
        ...
        async with httpx.AsyncClient(timeout=self.timeout) as client:
            ...
            agent_resp = await client.get(
                f"{self.registry_base_url}/v1/agents/{agent_name}", headers=self.headers
            )
            ...
            deps_resp = await client.get(
                f"{self.registry_base_url}/v1/deployments{query}", headers=self.headers
            )
