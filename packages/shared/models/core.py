...
from sqlalchemy import Column, String, Integer, Boolean, DateTime, JSON, ForeignKey, Text, Float, LargeBinary
...
class ApiKey(Base):
    ...
    key_hash: Mapped[str]
    name: Mapped[str]
    last_used_at: Mapped[Optional[datetime.datetime]]
    created_at: Mapped[datetime.datetime] = mapped_column(server_default=func.now())
...
class Agent(Base):
    ...
    is_active: Mapped[bool] = mapped_column(default=True)
    capabilities: Mapped[Optional[Dict[str, Any]]] = mapped_column(type_=JSON)
    created_at: Mapped[datetime.datetime] = mapped_column(server_default=func.now())
    
    # New fields for Agent Integration
    last_seen_at: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime(timezone=True))
...
class AgentCheckin(Base):
    ...
    status: Mapped[str]
    meta_data: Mapped[Optional[Dict[str, Any]]] = mapped_column(type_=JSON)
    created_at: Mapped[datetime.datetime] = mapped_column(server_default=func.now())
...
class PersonaVersion(Base):
    ...
    tools_policy: Mapped[Optional[Dict[str, Any]]] = mapped_column(type_=JSON)
    meta_data: Mapped[Optional[Dict[str, Any]]] = mapped_column(type_=JSON)
    created_at: Mapped[datetime.datetime] = mapped_column(server_default=func.now())
...
class Task(Base):
    ...
    persona_version_id: Mapped[Optional[int]] = mapped_column(ForeignKey("persona_versions.id"))
    requested_by_user_id: Mapped[Optional[int]] = mapped_column(ForeignKey("users.id"))
    requested_by_agent_id: Mapped[Optional[int]] = mapped_column(ForeignKey("agents.id"))
    assigned_agent_id: Mapped[Optional[int]] = mapped_column(ForeignKey("agents.id"))
...
class TaskRun(Base):
    ...
    error_text: Mapped[Optional[str]] = mapped_column(type_=Text)
    logs_object_key: Mapped[Optional[str]]
...
class TaskRoute(Base):
    ...
    preferred_agent_id: Mapped[Optional[int]] = mapped_column(ForeignKey("agents.id"))
    is_active: Mapped[bool] = mapped_column(default=True)
    
    # RAG routing rules
    needs_rag: Mapped[bool] = mapped_column(default=False)
    rag_namespaces: Mapped[Optional[List[str]]] = mapped_column(type_=JSON)
    rag_top_k: Mapped[Optional[int]]
...
class KbSource(Base):
    ...
    kind: Mapped[str]
    config: Mapped[Optional[Dict[str, Any]]] = mapped_column(type_=JSON)
    is_active: Mapped[bool] = mapped_column(default=True)
    created_at: Mapped[datetime.datetime] = mapped_column(server_default=func.now())
...
class Entity(Base):
    ...
    data: Mapped[Optional[Dict[str, Any]]] = mapped_column(type_=JSON)
...
class EntityEvent(Base):
    ...
    actor_id: Mapped[Optional[str]] # uuid string
    action: Mapped[str]
    before: Mapped[Optional[Dict[str, Any]]] = mapped_column(type_=JSON)
    after: Mapped[Optional[Dict[str, Any]]] = mapped_column(type_=JSON)
    diff: Mapped[Optional[Dict[str, Any]]] = mapped_column(type_=JSON)
    correlation_id: Mapped[Optional[str]]
    idempotency_key: Mapped[Optional[str]] = mapped_column(index=True)
    created_at: Mapped[datetime.datetime] = mapped_column(server_default=func.now())
...
class IdempotencyKey(Base):
    ...
    response: Mapped[Optional[Dict[str, Any]]] = mapped_column(type_=JSON)
    created_at: Mapped[datetime.datetime] = mapped_column(server_default=func.now())
...
class PbxTarget(Base):
    ...
    tags: Mapped[Optional[Dict[str, Any]]] = mapped_column(type_=JSON, server_default='[]')
    is_active: Mapped[bool] = mapped_column(default=True)
    created_at: Mapped[datetime.datetime] = mapped_column(server_default=func.now())
...
class PbxSnapshot(Base):
    ...
    summary: Mapped[Optional[Dict[str, Any]]] = mapped_column(type_=JSON)
    created_at: Mapped[datetime.datetime] = mapped_column(server_default=func.now())
...
class MonitoringSource(Base):
    ...
    auth_secret_id: Mapped[Optional[str]] = mapped_column(ForeignKey("secrets.id"))
    tags: Mapped[Optional[Dict[str, Any]]] = mapped_column(type_=JSON, server_default='[]')
    is_active: Mapped[bool] = mapped_column(default=True)
    created_at: Mapped[datetime.datetime] = mapped_column(server_default=func.now())
...
class CarrierTarget(Base):
    ...
    base_url: Mapped[Optional[str]]
    api_key_secret_id: Mapped[Optional[str]] = mapped_column(ForeignKey("secrets.id"))
    api_secret_secret_id: Mapped[Optional[str]] = mapped_column(ForeignKey("secrets.id"))
    tags: Mapped[Optional[Dict[str, Any]]] = mapped_column(type_=JSON, server_default='[]')
    is_active: Mapped[bool] = mapped_column(default=True)
    created_at: Mapped[datetime.datetime] = mapped_column(server_default=func.now())
