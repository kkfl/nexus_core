...
from packages.shared.models import IdempotencyKey
...
ALLOWED_ENTITIES = {"dns_record": {"required_fields": ["name", "record_type", "value", "ttl"]}}
    ...
    for field in config["required_fields"]:
        if field not in write.patch:
            raise SoRValidationError(
                f"Missing required field {field} for {write.entity_kind}", "sor_validation_failed"
            )


async def check_idempotency(
    db: AsyncSession, key: str, scope: str, request_data: dict[str, Any]
) -> dict[str, Any] | None:
    """
    ...
    if idem_record:
        if idem_record.request_hash == req_hash:
            ...
            raise SoRValidationError(
                "Idempotency conflict: key reused with different payload.", "idempotency_conflict"
            )
