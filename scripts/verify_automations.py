...
def trigger_and_wait(automation_id, name):
    ...
    r2 = requests.post(
        f"{BASE}/automations/{automation_id}/run", headers=HEADERS, json=run_payload, timeout=5
    )
...
def main():
    ...
    for a in autos:
        if a["name"] in (
            "Monitoring Targets Sync",
            "System Status Check",
            "System Status Daily Digest",
        ):
