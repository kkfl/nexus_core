...
@pytest.mark.skipif(os.environ.get("RUN_INTEGRATION_TESTS", "false").lower() != "true", reason="Integration test only")
def test_rbac_enforcement():
    ...
    res = requests.post(f"{base_url}/auth/login", data={"username": "reader@local.host", "password": "reader_password"})
    if res.status_code == 200:
        token = res.json().get("access_token")
        
        # Reader trying to create a Persona
        create_res = requests.post(f"{base_url}/personas", json={"name": "Hacked", "is_active": True}, headers={"Authorization": f"Bearer {token}"})
        assert create_res.status_code == 403, "RBAC regression: Reader is allowed to write Personas!"
...
def test_api_keys_are_hashed():
    ...
    with Session(engine) as session:
        ...
        for secret in secrets:
            assert "sk-" not in secret.ciphertext.decode('utf-8') if isinstance(secret.ciphertext, bytes) else "sk-" not in secret.ciphertext, "Security Regression: Secret appears plaintext!"
    ...
    with Session(engine) as session:
        ...
        if event:
            with pytest.raises(sqlalchemy.exc.ProgrammingError) as exc:
                ...
                session.execute(update(EntityEvent).where(EntityEvent.id == event.id).values(action="hacked"))
                session.commit()
            
            assert "append_only" in str(exc.value) or "trigger" in str(exc.value).lower() or "read-only" in str(exc.value).lower(), "Security Regression: SoR is mutable!"
