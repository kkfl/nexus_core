...
def main():
    ...
    # ── Auth ──────────────────────────────────────────────────
    section("AUTH")
    r = client.post(f"{API}/auth/login", data={"username": "admin@nexus.local", "password": "admin_password"})
    ...
    sources = r.json()
    if not sources:
        r = client.post(f"{API}/kb/sources", json={"name": "e2e-source", "kind": "manual"}, headers=headers)
    ...
    if url_doc:
        print(f"  A1 kb_documents: id={url_doc['id']} status={url_doc['ingest_status']} checksum={url_doc.get('checksum','?')[:16]}...")
        print(f"     storage_backend={url_doc.get('storage_backend')} object_key={url_doc.get('object_key')}")
        print(f"     version={url_doc.get('version')}")
    else:
        print(f"  WARN: No doc titled 'E2E URL Test Doc' found. Available: {[d['title'] for d in docs[:5]]}")
        if docs:
            url_doc = docs[-1]
    ...
    if url_doc:
        ...
        if chunks:
            c = chunks[0]
            print(f"     chunk[0]: index={c.get('chunk_index')} start_char={c.get('start_char')} end_char={c.get('end_char')} chars={c.get('char_count')}")
        ...
        r = client.post(f"{API}/kb/search", json={
        ...
        for sr in results[:2]:
            print(f"     doc_id={sr.get('document_id')} chunk_idx={sr.get('chunk_index')} start={sr.get('start_char')} end={sr.get('end_char')} score={sr.get('score','?')}")
    ...
    for ev in ev_data.get("events", [])[:2]:
        print(f"    id={ev.get('id','?')[:12]}... type={ev.get('event_type')} producer={ev.get('produced_by')}")
    ...
    if r.status_code == 200:
        ...
        if r.status_code == 200:
            ed = r.json()
            print(f"  B doc: id={ed['id']} status={ed['ingest_status']} storage={ed.get('storage_backend')} key={ed.get('object_key')}")
            print(f"     checksum={ed.get('checksum','?')[:16]}... meta={json.dumps(ed.get('meta_data',{}))[:200]}")
