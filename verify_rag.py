...
async def verify():
    ...
    async with get_db_context() as db:
        ...
        for doc in docs:
            ...
            print(f"  Storage: {doc.storage_backend} | Key: {doc.object_key} | Content-Type: {doc.content_type}")
            print(f"  Checksum: {doc.checksum}")
            
            # Verify MinIO
            try:
                raw_bytes = storage.get_bytes(doc.object_key)
                print(f"  [PASS] Successfully retrieved {len(raw_bytes)} bytes from MinIO for {doc.object_key}")
            except Exception as e:
                print(f"  [FAIL] Could not retrieve from MinIO: {e}")

            # Check embeddings metadata
            res_emb = await db.execute(select(KbEmbedding).join(KbEmbedding.chunk_id).where(KbEmbedding.chunk_id.in_(
                select(KbEmbedding.chunk_id).where(KbEmbedding.chunk_id.in_(
        ...
        for doc in docs:
            ...
            for c in chunks:
                ...
                if emb:
                    print(f"    Embedding: model='{emb.model}', dimension=(hardcoded 384 in schema)")
